      ...
      <section className="space-y-4">
        ...
        {chunks && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
            <p className="text-[12px] text-zinc-500">Top-3 chunks (<LessonRef id="b2" short />/<LessonRef id="b3" short />/<LessonRef id="b4" short />)</p>
            {chunks.map((c, i) => (
        ...
        {streamingOutput && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-lg border border-zinc-800 bg-zinc-900 px-4 py-3">
            <p className="text-[12px] text-zinc-500 mb-2">
              Answer (<LessonRef id="b1" short />/<LessonRef id="b5" short />/<LessonRef id="b6" short />)
        ...
        {log && (
          <div className="rounded border border-zinc-800 bg-zinc-950 px-3 py-2 font-mono text-[11px] text-zinc-600 leading-relaxed">
            <p className="text-[10px] text-zinc-700 mb-1">Eval log (<LessonRef id="b7" short />)</p>
        ...
        questions={[
          {
            ...
            options: [
              'Chat completion — the streamed response was generated by an LLM',
              'Embeddings — the query was embedded to find chunks',
              'Vector store — cosine similarity ranked the chunks',
              'None   all 7 concepts took part. The only thing not shown is a production eval suite running continuously.',
            ],
            answer: 3,
            explanation: (
              <>Every concept contributed: <LessonRef id="b1" /> generated the answer, <LessonRef id="b2" /> + <LessonRef id="b3" /> found chunks, <LessonRef id="b4" /> chunked the doc, <LessonRef id="b5" /> provided the system prompt, <LessonRef id="b6" /> streamed it, <LessonRef id="b7" /> logged it. A production system would add scheduled eval runs to catch regressions automatically.</>
            ),
          },
          {
