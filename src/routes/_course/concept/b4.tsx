...
const { prev, next } = getNeighbors('b4')
  ...
  return (
    <div className="max-w-3xl mx-auto px-8 py-10 pb-24">
      <header className="mb-8">
        ...
        <h1 className="mt-1 text-2xl font-semibold text-zinc-100">RAG — answer from your docs</h1>
      </header>
      ...
      <section className="space-y-4 text-[14px] leading-relaxed text-zinc-300">
        ...
        <ol className="space-y-3 mt-2 list-decimal ml-5">
          ...
          <li><strong className="text-zinc-200">Embed + store:</strong> turn each chunk into a vector, store it (<LessonRef id="b3" />).</li>
          <li><strong className="text-zinc-200">Retrieve:</strong> embed the user's question, find top-K closest chunks (<LessonRef id="b2" /> + <LessonRef id="b3" />).</li>
          <li><strong className="text-zinc-200">Generate:</strong> paste retrieved chunks + question into a chat prompt; the LLM answers grounded in them (<LessonRef id="b1" />).</li>
        </ol>
         <p className="text-[13px] text-zinc-500">Steps 2-3 you've already built. Step 4 is <LessonRef id="b1" /> + a smarter prompt. Step 1   chunking   is what this lesson focuses on, because it's the step beginners skip and seniors obsess over.</p>
        ...
        <p>
         ...
         <p className="text-[13px] text-zinc-500">This is the bare minimum. Senior-level RAG adds <em>reranking</em> (a second model re-orders top-K), <em>hybrid search</em> (BM25 + vector), and <em>eval sets</em> (<LessonRef id="b7" />) to catch regressions. We'll cover those in Phase C.</p>
      </section>
      ...
      <section className="mt-10">
        ...
        <UnderTheHood
          ...
          description="The RagPipeline composes the earlier foundations: chunk → embed → store → retrieve → chat-completion — the steps from How models work, Embeddings, and Vector stores. One fresh pipeline keeps state for one document set."
          language="tsx"
      ...
      <Quiz
        questions={[
          ...
          },
          {
            ...
            answer: 2,
            explanation: (
              <>No prompt can guarantee zero hallucination. But an explicit grounding instruction massively reduces the rate. Real production systems add a second LLM (a "critic") to check whether the answer is supported by the citations. We cover this in <LessonRef id="b7" />.</>
            ),
          },
          {
